...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, color }) => (
  <div className={`p-6 rounded-xl shadow-lg text-white ${color} print:bg-gray-100 print:text-black print:border print:border-gray-300 print:shadow-none`}>
...
const ChartCard: React.FC<{ title: string; children: React.ReactNode; }> = ({ title, children }) => (
  <div className="bg-white dark:bg-dark-card p-6 rounded-xl shadow-lg h-full flex flex-col min-h-[350px] print:shadow-none print:border print:border-gray-300 print:bg-white">
    <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-4 flex-shrink-0 print:text-black">{title}</h3>
...
const TransactionRow: React.FC<{ tx: Transaction; getEmployeeName: (id: string) => string }> = ({ tx, getEmployeeName }) => {
  ...
  const paymentMethodPill = (method: Transaction['paymentMethod']) => {
    ...
    const printStyles = 'print:bg-gray-200 print:text-black print:border print:border-gray-400';
    return <span className={`text-xs font-semibold px-3 py-1 rounded-full ${styles[method]} ${printStyles}`}>{method}</span>;
  };
  ...
  const statusPill = (status: Transaction['status']) => {
    ...
    const printStyles = 'print:bg-gray-200 print:text-black print:border print:border-gray-400';
    return <span className={`text-xs font-semibold px-3 py-1 rounded-full ${styles[status]} ${printStyles}`}>{status}</span>;
  };
  
  return (
    <>
      <tr onClick={() => setIsExpanded(!isExpanded)} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 print:bg-white">
        <td className="px-5 py-4 text-sm">
          <p className="text-gray-900 dark:text-gray-300 whitespace-no-wrap print:text-black">{tx.id}</p>
        </td>
        <td className="px-5 py-4 text-sm">
          <p className="text-gray-900 dark:text-gray-300 whitespace-no-wrap print:text-black">{new Date(tx.date).toLocaleString('pt-BR')}</p>
        </td>
        <td className="px-5 py-4 text-sm">
          <p className="text-gray-900 dark:text-gray-300 whitespace-no-wrap print:text-black">{getEmployeeName(tx.employeeId)}</p>
        </td>
        ...
        <td className="px-5 py-4 text-sm text-right">
          <p className="text-gray-900 dark:text-gray-300 whitespace-no-wrap font-semibold print:text-black">{formatCurrency(tx.total)}</p>
        </td>
      </tr>
      {isExpanded && (
        <tr className="bg-gray-50 dark:bg-gray-800/50 print:bg-gray-50">
          <td colSpan={6} className="p-4">
            <div className="text-sm text-gray-700 dark:text-gray-300 print:text-gray-800">
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1 print:text-gray-600">
...
const Reports: React.FC<ReportsProps> = ({ allTransactions, employees, cashDrawerOperations }) => {
  ...
  return (
    <div className="p-8 bg-light-bg dark:bg-dark-bg h-full overflow-y-auto print:p-4 print:bg-white">
      ...
      {reportGenerated && (
        <div id="report-content" className="space-y-8">
            <div className="hidden print:block text-center mb-8">
                <h1 className="text-3xl font-bold text-black">Relatório de Vendas</h1>
                {(startDate || endDate) && 
                    <p className="text-sm text-gray-600">
                        Período de {startDate ? new Date(startDate).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : '...'} a {endDate ? new Date(endDate).toLocaleDateString('pt-BR', {timeZone: 'UTC'}) : '...'}
                    </p>
                }
            </div>
          ...
          {reportData.length > 0 || reportDrawerOps.length > 0 ? (
            <>
                          ...
                          {dailySales.map(day => (
                            <div key={day.date} className="flex flex-col items-center flex-1 h-full text-center">
                              <div className="h-full w-full flex items-end justify-center">
                                <div className="bg-blue-400 w-4/5 rounded-t-md hover:bg-blue-500 transition-colors print:bg-gray-500" style={{ height: `${(day.total / Math.max(...dailySales.map(d => d.total), 1)) * 100}%` }} title={`${day.date}: ${formatCurrency(day.total)}`}></div>
                              </div>
                              <span className="text-xs text-gray-500 dark:text-gray-400 mt-2 whitespace-nowrap print:text-black">{day.date}</span>
                            </div>
                          ))}
                        </div>
                    ) : (
                        <p className="text-gray-500 dark:text-gray-400 print:text-gray-600">Sem dados de vendas para o gráfico.</p>
                    )}
                  </ChartCard>
                </div>
                        ...
                        {salesByPaymentMethod.map(method => (
                          <div key={method.name}>
                              <div className="flex justify-between items-center text-sm mb-1">
                                <span className="flex items-center text-gray-600 dark:text-gray-300 print:text-black">
                                  <span className={`w-3 h-3 rounded-full mr-2 ${paymentColors[method.name] || 'bg-gray-400'} print:bg-gray-500`}></span>
                                  {method.name}
                                </span>
                                <span className="font-semibold text-gray-800 dark:text-gray-200 print:text-black">{formatCurrency(method.value)}</span>
                              </div>
                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 print:bg-gray-200">
                                <div 
                                    className={`${paymentColors[method.name] || 'bg-gray-400'} h-2 rounded-full print:bg-gray-600`}
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-4 print:text-black">Transações Detalhadas</h2>
                <div className="bg-white dark:bg-dark-card shadow-md rounded-lg overflow-hidden print:shadow-none print:border print:border-gray-300">
                  <table className="min-w-full leading-normal">
                    <thead className="bg-gray-100 dark:bg-gray-800 print:bg-gray-200">
                      <tr className="border-b-2 border-gray-200 dark:border-gray-700 print:border-gray-400 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider print:text-black">
            ...
            <div className="text-center py-10 bg-white dark:bg-dark-card rounded-lg shadow-md print:shadow-none print:border print:border-gray-200">
              <p className="text-gray-500 dark:text-gray-400 print:text-gray-600">Nenhum dado para o período e filtro selecionados. Ajuste os filtros e gere um novo relatório.</p>
            </div>
          )}
        </div>
      )}
